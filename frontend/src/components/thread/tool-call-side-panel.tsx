...
// Helper function to generate the console title
const getComputerTitle = (agentName?: string): string => {
  return agentName ? `${agentName}'s Console` : "Xera Console";
};
  ...
  if (variant === 'motion') {
    return (
      <motion.div
        layoutId={layoutId}
        className="p-3"
        data-has-tool-result={hasToolResult ? '' : undefined}
      >
        <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-center space-y-4 max-w-sm text-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
                  Tool calls and console activity will appear here when they're being executed.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pointer-events-none absolute top-2 right-2 flex flex-col items-end gap-1 text-[11px] font-medium">
            ...
            {displayToolCall && (
              <div className="px-2 py-1 rounded-md bg-black/60 text-white backdrop-blur-sm max-w-[240px] truncate">
                {isStreaming ? 'Running: ' : ''}
                {getUserFriendlyToolName(displayToolCall.assistantCall.name)}
              </div>
            )}
          </div>
          <div className="flex-1 flex flex-col overflow-hidden">
