export const AGENTPRESS_TOOL_DEFINITIONS: Record<string, { enabled: boolean; description: string; icon: string; color: string }> = {
  // Core sandbox tools (legacy sb_ keys retained for compatibility)
...
// Helper to normalize tool keys for display without sb_ prefix, while keeping data keys unchanged
export const normalizeToolKey = (toolKey: string): string => {
  return toolKey.startsWith('sb_') ? toolKey.slice(3) : toolKey;
};

export const getToolDisplayName = (toolName: string): string => {
    // Normalize legacy keys by stripping sb_ prefix
    const normalize = (name: string) => name.startsWith('sb_') ? name.slice(3) : name;
    const key = normalize(toolName);
    const displayNames: Record<string, string> = {
      // Core sandbox tools
      'shell_tool': 'Terminal',
      'files_tool': 'File Manager',
      'deploy_tool': 'Deploy',
      'expose_tool': 'Expose Port',
      'web_search_tool': 'Web Search',
      'vision_tool': 'Image Processing',
      'image_edit_tool': 'Image Editor',
      'presentation_outline_tool': 'Presentation Outline',
      'presentation_tool': 'Presentation Creator',
      'docs_tool': 'Document Editor',

      'sheets_tool': 'Spreadsheets',
