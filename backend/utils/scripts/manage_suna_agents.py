#!/usr/bin/env python3
"""
Xera Default Agent Management Script (Simplified)

This script provides administrative functions for managing Xera default agents across all users.
...
Note: Sync is no longer needed - Xera agents automatically use the current configuration from config.py
"""
...
class SunaAgentManager:
    ...
    async def install_all_users(self):
        """Install Xera agent for all users who don't have it"""
        print("🚀 Installing Xera default agent for all users who don't have it...")
    ...
    async def update_config_info(self):
        ...
        print("ℹ️  Xera Configuration Information")
        print("=" * 50)
        print("🔧 Xera agents automatically use the current configuration from config.py")
        print("📝 No sync needed - changes are applied immediately when agents run")
        print("💡 To update Xera behavior, simply modify backend/agent/suna/config.py")
        print("\n✅ All Xera agents are always up-to-date with your latest configuration!")
    
    async def install_user(self, account_id):
        """Install Xera agent for specific user"""
        print(f"🚀 Installing Xera default agent for user {account_id}...")
        
        agent_id = await self.service.install_suna_agent_for_user(account_id)
        
        if agent_id:
            print(f"✅ Successfully installed Xera agent {agent_id} for user {account_id}")
        else:
            print(f"❌ Failed to install Xera agent for user {account_id}")

    async def replace_user_agent(self, account_id):
        """Replace Xera agent for specific user (in case of corruption)"""
        print(f"🔄 Replacing Xera agent for user {account_id}...")
        ...
        if agent_id:
            print(f"✅ Successfully replaced Xera agent {agent_id} for user {account_id}")
        else:
            print(f"❌ Failed to replace Xera agent for user {account_id}")

    async def show_stats(self):
        """Show Xera agent statistics"""
        print("📊 Xera Default Agent Statistics")
        print("=" * 50)
...
async def main():
    parser = argparse.ArgumentParser(
        description="Manage Xera default agents across all users (Simplified)",
    ...
    # Main commands
    subparsers.add_parser('install-all', help='Install Xera agent for all users who don\'t have it')
    subparsers.add_parser('stats', help='Show Xera agent statistics')
